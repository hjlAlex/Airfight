...
(function (game) {
    ...
    var GameContainer = (function (_super) {
        ...
        function GameContainer() {
            ...
            /**触发创建buff的间隔*/
            _this.buffTimer = new egret.Timer(30000);
            /**敌人的子弹*/
            _this.enemyBullets = [];
            _this.dieimages = [];
            /**我的成绩*/
        ...
        GameContainer.prototype.gameViewUpdate = function (evt) {
            ...
            //生命buff运动
            var buffCount = this.bloodBuff.length;
            var buff;
        ...
        GameContainer.prototype.createEnemyAir = function () {
            ...
            while (this.dieimages.length > 0) {
                var dimage = this.dieimages.pop();
                if (this.contains(dimage)) {
                    this.removeChild(dimage);
                }
            }
        };
        GameContainer.prototype.createBuff = function () {
        ...
        GameContainer.prototype.gameHitJudge = function () {
                ...
                for (var j = 0; j < enemyAirCount; j++) {
                    ...
                    if (game.util.GameUtil.hitTest(bullet, enemyAir)) {
                        ...
                        if (enemyAir.getBlood() <= 0 && delFighters.indexOf(enemyAir) == -1) {
                            ...
                            /**敌机死亡发出音效 */
                            var dieMusic = RES.getRes("effcet_enemydie_mp3");
                            dieMusic.play(0, 1);
                        }
                    }
                }
            ...
            for (var i = 0; i < buffCount; i++) {
                buff = this.bloodBuff[i];
                if (game.util.GameUtil.hitTest(buff, this.myAirfight)) {
            ...
            if (this.myAirfight.getBlood() <= 0) {
                ...
                while (delFighters.length > 0) {
                    ...
                    var dieimage = void 0;
                    var dieimageX = enemyAir.x;
                    var dieimageY = enemyAir.y;
                    if (this.contains(enemyAir)) {
                        this.removeChild(enemyAir);
